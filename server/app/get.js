...
app.get("/db/config",async (req,res)=>{
    ...
    let config = {}; db_config.map(i=>config[i.key] = i.value);
    res.json(config);
});
...
app.get("/db/tests/:testcode", async ({params},res)=>{
    res.json(await models.test.findOne({ raw:true,where:{testcode:params.testcode}}));
});

// GET TEST HISTORY
app.get("/db/tests/:testcode/history", async (req,res)=>{
    res.json(await models.history.findAll({ raw:true,where:{testcode:req.params.testcode}}));
});

// GET TEST HISTORY BY CODE
app.get("/db/tests/:testcode/history/:code", async ({params},res)=>{
    res.json(await models.history.findAll({ raw:true,where:{code:params.code,testcode:params.testcode}}));
});

// GET TEST QUESTION
app.get("/db/tests/:testcode/question", async ({params},res)=>{
    res.json(await models.question.findAll({ raw:true,where:{testcode:params.testcode}}));
});
...
app.get("/db/tests/:testcode/total", async ({params},res)=>{
    res.json(await models.total.findAll({raw:true,where:{testcode:params.testcode}}));
});

// GET TEST TOTAL BY CODE
app.get("/db/tests/:testcode/total/:code", async ({params},res)=>{
    res.json(await models.total.findAll({raw:true,where:{testcode:params.testcode,code:params.code}}));
});

// GET TEST TOTAL 
app.get("/db/total/:code",async ({params},res)=>{
    res.json(await models.total.findAll({raw:true,where:{code:params.code}}));
});
